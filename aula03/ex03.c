...
Celula buscaInimigoMaisProximo(Celula atual) {
  ...
  int type = (atual->personagem.tipo == JOGADOR) ? MONSTRO : JOGADOR;

  Celula aux = atual->prox;

  while(aux != atual) {
    if(aux->personagem.tipo == type){
      printf("Contato!, um %s!!\n\n", aux->personagem.nome);
      return aux;
    }
    aux = aux->prox;
...
Celula executaUmTurno(ListaCircular lista, Celula atual) {
  ...
  if(atual->personagem.tipo == JOGADOR && alvo->personagem.tipo == JOGADOR || atual->personagem.tipo == MONSTRO && alvo->personagem.tipo == MONSTRO){
    alvo = atual->prox;
  } else if (atual->personagem.tipo == JOGADOR && alvo->personagem.tipo == MONSTRO || atual->personagem.tipo == MONSTRO && alvo->personagem.tipo == JOGADOR){
    printf("É a vez do %s\n\n", atual->personagem.nome);
    unsigned int seed = time(0);
    int maxDado = atual->personagem.iniciativa;
    int minDado = 1; 
    int dado = rand_r(&seed) % (maxDado - minDado + 1) + minDado;
    printf("Dado: %d\n\n", dado);
    if(dado >= atual->personagem.iniciativa){
      printf("Hit sucedido!\n\n");
      int danoMax = atual->personagem.dano;
      int danoMin = atual->personagem.dano/2;
      int recebe = rand_r(&seed) % (danoMax - danoMin + 1) + danoMin;
      alvo->personagem.vida -= recebe;
      printf("Dano dado por %s: %d -- Para: %s \n\n", atual->personagem.nome, recebe, alvo->personagem.nome);
      if(alvo->personagem.vida <= 0){
        printf("%s Morreu!\n\n", alvo->personagem.nome);
        removeDaListaCircular(lista, alvo);
      }
    } else if (dado < atual->personagem.iniciativa) {
      printf("Errou!\n\n");
      atual = atual->prox;
    }
  }
...
void teste2_um_turno_sem_morte(void) {
  ...
  removeDaListaCircular(lista, lista->inicio);
